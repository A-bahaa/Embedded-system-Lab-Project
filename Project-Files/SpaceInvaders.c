...
int main(void){
    ...
    Nokia5110_Init(); // initialize the screen
    vertical_pos1 = CARH, vertical_pos2 =  CARH - 24 ; // initialize the vertical positions of the two enemy groups
    horizontal_pos = 32;    // the start pos of the player's car
    r1 = 1;    
    r2 = 3;
    score = 0;  
  ...
  while(1){
	if((ADC0_RIS_R & (1 << 3)) == (1 << 3)){ // if third bit is set that means there's a new input signal to be processed (the slider was moved ) 
	 	ADC0_ISC_R  |= (1<<3);  // clear the interrupt
		addData = ADC0_SSFIFO3_R;
	}
	horizontal_pos = (addData * (80-CARSW))/4095;  // update the position of the player
	...
	vertical_pos1 += ((score/10) + 1); // step the enemies down to get the illusion of movement
	...
	if(vertical_pos1 > 37){
		...
		if ((horizontal_pos >= 60 && horizontal_pos <68 && r1 & (1<<0)) || (horizontal_pos >= 44 && horizontal_pos <52 && r1 & (1<<1)) ||(horizontal_pos >= 28 && horizontal_pos <36 && r1 & (1<<2)) || (horizontal_pos >= 12 && horizontal_pos <20 && r1 & (1<<3)) || (horizontal_pos >= 0 && horizontal_pos <8 && r1 & (1<<4))){	
			//Delay100ms(10);
			print_(score);  // jump to game-over
			break;
		}
	}
...
// In case of using switches instead of the ADC slider
